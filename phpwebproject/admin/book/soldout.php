<?php
     ...
     $sql="select book.*,class.name cname from book,class where book.class_id=class.id and book.shelf=0 order by book.id";
     $rst=mysql_query($sql);
     ...
     if($hangnum == 0){
        ...
        $fenye_sel = "select book.*,class.name cname from book,class where book.class_id=class.id and book.shelf=0 order by book.id limit $start,$size";
